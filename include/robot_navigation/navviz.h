...
class NavVisualizer
{
  public:
    ...
    void visualizeConfiguration(double color_num, const std::vector< vector<double> > &coords, std::string name);
    ...
    void visualizeCircle_obs(std::vector<double> pose, int color, std::string text, double radius);
    ...
    void visualizePointsRGB(const std::vector<std::vector<double> > &pose, std::string text, double r, double g, double b);
...
void NavVisualizer::visualizeComplexObstacles(const std::vector<std::vector<double> > &obstacles, std::string *objName, int color, double trans)
{
  ...
  for(int i = 0; i < int(obstacles.size()); i++)
  {
    ...
    marker_array_.markers[i].color.b = b;//0.5; HSVtoRGB
...
void NavVisualizer::visualizeConfigurations(std::string name, double scolor, double ecolor, const std::vector<std::vector<double> > &traj, int throttle)
{
  ...
  for(unsigned int i = 0; i < traj.size(); i+=throttle){  
    ...
    pose.pose.orientation.w = cos((traj[i][2] + ((2.0*22.0) / (4.0 * 7.0))) / 2.0);
    pose.pose.orientation.x = 0;
    pose.pose.orientation.y = 0;
    pose.pose.orientation.z = sin((traj[i][2] + ((2.0 * 22.0) / (4.0 * 7.0))) / 2.0);
...
void NavVisualizer::visualizeCircle(std::vector<double> pose, int color, std::string text, double radius)
{
  ...
  double angle_step = (2.0*22.0/7.0) / 30.0f;
  for (int k = 0; k < 31; k++)
  {
...
void NavVisualizer::visualizeCircle_obs(std::vector<double> pose, int color, std::string text, double radius)
{
  advertiseTransform();
  visualization_msgs::Marker obs_marker;
  obs_marker.header.frame_id = reference_frame_;
...
void NavVisualizer::visualizePointsRGB(const std::vector<std::vector<double> > &pose, std::string text, double r, double g, double b)
{
  advertiseTransform();
  visualization_msgs::Marker marker;
...
void NavVisualizer::visualizeMesh(double color_num, std::vector<geometry_msgs::PoseStamped> &poses, std::string name)
{
  ...
  for(int i = 0; i < (int)marker_array_.markers.size(); ++i)
  {
    ...
    marker_array_.markers[i].scale.x = 0.02;  
    marker_array_.markers[i].scale.y = 0.02;
...
void NavVisualizer::visualizeConfiguration(double color_num, const std::vector< vector<double> > &coords, std::string name)
{
  advertiseTransform();
  ...
  poses.resize(coords_size);
  //poses.resize(1);
  geometry_msgs::PoseStamped pose;
  
  //update the poses to the new frame
  for(unsigned int i = 0; i < poses.size(); i++){  
    poses[i].pose.position.x = coords[i][0];
    poses[i].pose.position.y = coords[i][1];
    poses[i].pose.position.z = 0.0f;

    poses[i].pose.orientation.w = cos((coords[i][2] + ((2.0 * 22.0/7.0) / 4.0)) / 2.0);
    poses[i].pose.orientation.x = 0;
    poses[i].pose.orientation.y = 0;
    poses[i].pose.orientation.z = sin((coords[i][2] + ((2.0 * 22.0/7.0) / 4.0)) / 2.0);
  }
...
void NavVisualizer::visualizeLine(std::vector<double> p1, std::vector<double> p2, int color, std::string name){
  ...
  obs_marker.action = 0;
  int k = 10;
  obs_marker.scale.x = robot_width_ / (k*1.0);
  obs_marker.scale.y = robot_width_ / (k*1.0);
  obs_marker.scale.z = 0.025;
  double r=0,g=0,b=0;
  HSVtoRGB(&r, &g, &b, color, 1.0, 1.0);
  obs_marker.color.r = r;
